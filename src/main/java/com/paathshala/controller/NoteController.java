...
@PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<NoteResponse> createNote(@Valid @RequestBody NoteRequest request,@PathVariable String courseTitle)
{
    String decodedCourseTitle = URLDecoder.decode(courseTitle,StandardCharsets.UTF_8);
    NoteResponse response = contentService.addNote(request,courseTitle);
    ...
    public ResponseEntity<NoteResponse> getNote(@PathVariable String contentTitle)
{
    ...
    NoteResponse response = contentService.getNoteByTitle(decodedContentTitle);
    HttpHeaders header = new HttpHeaders();
    header.set("Content-Type","application/json");
    if(!response.isError())
